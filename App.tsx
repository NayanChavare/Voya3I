...
const ContextImage = ({ prompt, className = "", overlay = true }: { prompt: string, className?: string, overlay?: boolean }) => {
  ...
  return (
    <div className={`relative group overflow-hidden bg-slate-100 dark:bg-slate-800 flex items-center justify-center ${className}`}>
      ...
      {overlay && <div className="absolute inset-0 bg-slate-900/10 mix-blend-multiply" />}
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 animate-slide-up">
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl mr-4">
            <button 
              ...
              className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'results' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400'}`}
            >
              Results
            </button>
            <button 
              onClick={() => setActiveTab('users')}
              className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'users' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400'}`}
            >
              Users
            </button>
          </div>
          ...
          {activeTab === 'users' && (
            <button 
              ...
              className="px-6 py-3 bg-blue-600 text-white rounded-xl font-black flex items-center gap-2 hover:bg-blue-700 transition-all shadow-lg active:scale-95"
            >
      ...
      {activeTab === 'results' ? (
        ...
        <div className="bg-white dark:bg-slate-900/50 p-10 rounded-[3rem] shadow-xl border border-slate-100 dark:border-slate-800 overflow-hidden">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10">
            <h3 className="text-2xl font-black flex items-center gap-3 text-slate-900 dark:text-white"><Users2 className="text-blue-500" /> User Directory</h3>
            <div className="relative w-full md:w-80">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
              <input 
                type="text" 
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800">
                  <th className="pb-6 text-[10px] font-black text-slate-400 uppercase tracking-widest pl-4">User</th>
                  <th className="pb-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Role</th>
                  <th className="pb-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Institution</th>
                  <th className="pb-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Joined</th>
                  <th className="pb-6 text-[10px] font-black text-slate-400 uppercase tracking-widest pr-4 text-right">Actions</th>
                </tr>
              </thead>
              <tbody>
      ...
      {showUserModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[3rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden animate-slide-up">
            <header className="p-10 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
              <div>
                <h3 className="text-2xl font-black text-slate-900 dark:text-white">{editingUser ? 'Update User' : 'Create New User'}</h3>
                <p className="text-xs text-slate-500 font-bold mt-1">Manage institutional access.</p>
              </div>
              <button onClick={() => setShowUserModal(false)} className="p-3 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
                <X size={24} />
              </button>
            </header>
            ...
            <form onSubmit={handleSaveUser} className="p-10 space-y-6">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-blue-500 transition-all"
              ...
              <div className="space-y-2">
                ...
                <select 
                  ...
                  className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-blue-500 transition-all appearance-none"
                >
              ...
              <button 
                disabled={loading}
                type="submit" 
                className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black text-lg shadow-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50"
              >
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 animate-slide-up">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 mb-12">
        ...
        <div className="flex gap-4 no-print flex-wrap">
          <button onClick={() => onAcceptPathway(currentInsights!)} className="px-6 py-3 bg-slate-900 dark:bg-slate-800 text-white rounded-xl font-black flex items-center gap-2 hover:bg-black transition-all shadow-lg active:scale-95">
        ...
        <div className="lg:col-span-2 bg-slate-900 p-12 rounded-[3.5rem] shadow-2xl text-white relative overflow-hidden flex flex-col justify-between">
          {loadingInsights ? (
            ...
            <div className="relative z-10 h-full flex flex-col">
              ...
              <p className="text-xl text-slate-300 italic mb-10">"{currentInsights.summary}"</p>
              ...
              <div className="mt-auto pt-8 border-t border-slate-800 flex justify-between items-center no-print">
                ...
                <button onClick={() => onAcceptPathway(currentInsights)} className="px-10 py-5 bg-emerald-600 rounded-2xl font-black hover:bg-emerald-700 active:scale-95 transition-all shadow-xl flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {(results || []).map((res) => (
              <div 
                ...
                className="p-6 rounded-3xl border border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/20 group relative hover:border-emerald-500 transition-all overflow-hidden cursor-pointer"
              >
                ...
                <button 
                  ...
                  className="text-rose-500 opacity-0 group-hover:opacity-100 transition-all absolute bottom-6 right-6 hover:scale-110 no-print"
                >
                  <Trash2 size={18} />
                </button>
              </div>
...
const ResultSummary = ({ result, insights, onContinue, onReview, onSave, isSaving, autoSave, onToggleAutoSave, loading }: { result: AssessmentResult | null, insights: AIInsights | null, onContinue: () => void, onReview: () => void, onSave?: () => void, isSaving?: boolean, autoSave?: boolean, onToggleAutoSave?: (v: boolean) => void, loading?: boolean }) => {
  if (loading || !result) {
    return (
      <div className="max-w-6xl mx-auto px-6 py-32 flex flex-col items-center justify-center text-center animate-slide-up">
        <div className="relative mb-16">
          <div className="w-48 h-48 bg-emerald-500/10 rounded-full animate-ping absolute inset-0" />
          <div className="w-48 h-48 bg-emerald-500/20 rounded-full animate-pulse absolute inset-0" />
          <div className="relative w-48 h-48 bg-white dark:bg-slate-900 rounded-full shadow-2xl flex items-center justify-center border-4 border-emerald-500/30">
            <BrainCircuit size={80} className="text-emerald-500 animate-bounce" />
          </div>
        </div>
        <h1 className="text-6xl font-black tracking-tighter mb-6 text-slate-900 dark:text-white">
        ...
        <div className="max-w-md mx-auto space-y-6">
          ...
          <div className="flex justify-center gap-8 pt-8">
            <div className="flex flex-col items-center gap-2">
              <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-emerald-500 shadow-sm">
                <Database size={24} />
              </div>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Scoring</span>
            </div>
            <div className="flex flex-col items-center gap-2">
              <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-blue-500 shadow-sm">
                <Sparkles size={24} />
              </div>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Insights</span>
            </div>
            <div className="flex flex-col items-center gap-2">
              <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-amber-500 shadow-sm">
                <Target size={24} />
              </div>
  ...
  const isPreview = result.id === 'preview';
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12 animate-slide-up">
      <div className="text-center mb-16 relative">
        ...
        <h1 className="text-7xl font-black tracking-tighter mb-4 text-slate-900 dark:text-white">
          {isPreview ? 'Analysis Ready' : 'Success Unlocked'}
        </h1>
        <p className="text-2xl text-slate-500 font-bold max-w-2xl mx-auto leading-tight">
          Your SDG impact profile for <span className="text-emerald-500">{result.dept}</span> has been synthesized.
          {isPreview && <span className="block mt-2 text-amber-500 text-sm uppercase tracking-widest">Preview Mode - Not Yet Saved</span>}
        </p>
      </div>
        ...
        {isPreview ? (
          <div className="flex-1 flex flex-col gap-4">
            <button 
              onClick={onSave} 
              disabled={isSaving}
              className="w-full py-8 bg-emerald-600 text-white rounded-[2.5rem] font-black text-2xl shadow-2xl hover:bg-emerald-700 transition-all active:scale-95 flex items-center justify-center gap-4 group disabled:opacity-50"
            >
              {isSaving ? <RefreshCw className="animate-spin" size={32} /> : <CloudUpload size={32} />}
              {isSaving ? 'Archiving...' : 'Save to Profile'}
            </button>
            <label className="flex items-center justify-center gap-3 cursor-pointer group">
              ...
              />
              <span className="text-sm font-black text-slate-500 uppercase tracking-widest group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Always Auto-Save Results</span>
            </label>
          </div>
        ) : (
          <button onClick={onContinue} className="flex-1 py-8 bg-emerald-600 text-white rounded-[2.5rem] font-black text-2xl shadow-2xl hover:bg-emerald-700 transition-all active:scale-95 flex items-center justify-center gap-4 group">
            View Full Report <ArrowRight size={32} className="group-hover:translate-x-3 transition-transform" />
          </button>
        )}
        <button onClick={onReview} className="flex-1 py-8 bg-white dark:bg-slate-900 text-slate-900 dark:text-white border-2 border-slate-200 dark:border-slate-800 rounded-[2.5rem] font-black text-2xl shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-all active:scale-95 flex items-center justify-center gap-4">
            ...
            {dept === 'Other' && (
              <div className="mt-8 animate-slide-up">
                ...
                <select 
                  ...
                  className="w-full px-8 py-5 bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-emerald-500 rounded-3xl font-bold outline-none transition-all shadow-inner text-xl text-slate-900 dark:text-white cursor-pointer"
                >
              ...
              <div className="space-y-8 animate-slide-up">
                <div className="p-10 bg-slate-900 rounded-[3rem] text-white flex flex-col md:flex-row items-center gap-8 relative overflow-hidden group">
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-2xl font-bold leading-tight italic">"{suggestedSlogan}"</p>
                  </div>
                </div>
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 animate-slide-up">
      ...
      <div className="no-print">
        ...
        <section className="mb-16 grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 bg-slate-900 p-10 rounded-[4rem] shadow-2xl text-white flex flex-col justify-center relative overflow-hidden">
            ...
            <p className="text-xl text-slate-300 italic leading-relaxed relative z-10">"{insights.summary}"</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-12">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {(insights?.impactAnalysis || []).map((finding, i) => (
                    <div key={i} className="group p-8 bg-slate-50 dark:bg-slate-800/40 rounded-[3rem] border border-slate-100 dark:border-slate-700 flex flex-col h-full hover:shadow-2xl transition-all">
                      <ContextImage prompt={finding.visualPrompt || `${finding.area} ${finding.status}`} className="w-full h-48 rounded-[2rem] mb-6 shrink-0" />
            ...
            <section className="bg-white dark:bg-slate-900/50 p-12 rounded-[4rem] shadow-xl border border-slate-100 dark:border-slate-800">
               <div className="flex items-center justify-between mb-10">
                  <h2 className="text-3xl font-black flex items-center gap-4 text-slate-900 dark:text-white"><ShieldAlert className="text-rose-500" size={32} /> Risk Mitigation Layer</h2>
               ...
               <div className="space-y-6">
                  {(insights?.riskMitigation || []).map((risk, i) => (
                    <div key={i} className="p-8 bg-rose-50/30 dark:bg-rose-900/10 rounded-[3rem] border border-rose-100 dark:border-rose-900/30 flex flex-col md:flex-row gap-8 group hover:shadow-lg transition-all">
                       <ContextImage prompt={risk.visualPrompt || `${risk.risk} ${risk.impactLevel === 'High' ? 'critical risk' : 'warning'}`} className="w-full md:w-48 h-48 rounded-[2.5rem] shrink-0" />
  ...
  return (
    <div className="max-w-7xl mx-auto px-8 py-24 animate-slide-up">
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 ${roles.length === 3 ? 'lg:grid-cols-3' : 'lg:grid-cols-4'} gap-8`}>
        {roles.map(r => (
          <div key={r.id} onClick={() => onSelect(r.id)} className="group relative bg-white dark:bg-slate-900 p-12 rounded-[4.5rem] border-2 border-slate-100 dark:border-slate-800 hover:border-emerald-500 transition-all shadow-xl text-left flex flex-col items-center text-center overflow-hidden cursor-pointer">
  ...
  if (verificationSent) {
    return (
      <div className="max-w-2xl mx-auto px-8 py-24 animate-slide-up">
        <div className="bg-white dark:bg-slate-900 p-16 rounded-[5rem] shadow-2xl border border-slate-100 dark:border-slate-800 text-center">
          <div className="w-24 h-24 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center text-emerald-500 mx-auto mb-8 animate-bounce">
            <Mail size={48} />
          </div>
          <h2 className="text-4xl font-black text-slate-900 dark:text-white mb-4 tracking-tighter">Verify Your Email</h2>
          <p className="text-slate-500 dark:text-slate-400 font-bold text-lg mb-10">
          ...
          <div className="space-y-4">
            <button 
              ...
              className="w-full py-6 bg-slate-900 dark:bg-emerald-600 text-white rounded-3xl font-black text-xl shadow-xl hover:bg-black transition-all active:scale-95"
            >
              PROCEED TO LOGIN
            </button>
            <button 
              ...
              disabled={loading}
              className="w-full py-6 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-3xl font-black text-sm uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center justify-center gap-2"
            >
  ...
  return (
    <div className="max-w-2xl mx-auto px-8 py-24 animate-slide-up">
      ...
      <div className="bg-white dark:bg-slate-900 p-16 rounded-[5rem] shadow-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden">
        ...
        {error && (
          <div className="mb-10 p-8 bg-rose-50 dark:bg-rose-900/20 border-2 border-rose-100 dark:border-rose-900/30 rounded-[2.5rem] flex items-center gap-6 animate-shake shadow-lg">
            <div className="w-12 h-12 bg-rose-500 rounded-2xl flex items-center justify-center text-white shrink-0 shadow-lg">
              <ShieldAlert size={24} />
            </div>
            <div className="flex-1">
              <h4 className="text-xs font-black text-rose-500 uppercase tracking-widest mb-1">Security Alert</h4>
              <p className="text-sm font-bold text-rose-600 dark:text-rose-400 leading-tight">{error}</p>
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <>
              ...
              <div className="space-y-3">
                ...
                <select 
                  ...
                  className="w-full px-8 py-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all text-xl text-slate-900 dark:text-white appearance-none"
                >
              ...
              <div className="space-y-3 relative">
                ...
                <div className="relative">
                  <input 
                    ...
                    className={`w-full px-8 py-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold outline-none border-2 transition-all text-xl text-slate-900 dark:text-white ${!country ? 'opacity-50 cursor-not-allowed' : 'focus:border-emerald-500'} border-transparent`} 
                  ...
                  <div className="absolute z-50 left-0 right-0 mt-2 bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-700 max-h-60 overflow-y-auto custom-scrollbar">
                    {universities.map((u, i) => (
                      <button 
                        ...
                        className="w-full px-8 py-4 text-left hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors font-bold text-slate-700 dark:text-slate-200 border-b border-slate-50 dark:border-slate-700 last:border-0"
                      >
                        {u}
                      </button>
                    ))}
                ...
                {showAddForm && (
                  <div className="mt-6 p-8 bg-emerald-50 dark:bg-emerald-900/20 rounded-[2.5rem] border-2 border-emerald-100 dark:border-emerald-800 animate-in fade-in slide-in-from-top-4 duration-300">
                    <h4 className="text-sm font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest mb-6">Register New Institution</h4>
                    <div className="space-y-4">
                      <input 
                        type="text" 
                        ...
                        className="w-full px-6 py-4 bg-white dark:bg-slate-800 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all text-sm"
                      />
                      <input 
                        type="text" 
                      ...
                      <div className="flex gap-3">
                        <button 
                          ...
                          className="flex-1 py-4 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-500 transition-all disabled:opacity-50"
                        >
                        ...
                        <button 
                          ...
                          className="px-6 py-4 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-300 transition-all"
                        >
                          CANCEL
                        </button>
                      </div>
          ...
          <button disabled={loading || (isSignup && !isValidUniversity)} type="submit" className="w-full py-8 bg-slate-900 dark:bg-emerald-600 text-white rounded-[2.5rem] font-black text-2xl shadow-2xl hover:bg-black dark:hover:bg-emerald-500 transition-all flex items-center justify-center gap-4 active:scale-95 group disabled:opacity-50 disabled:cursor-not-allowed">
            {loading ? <RefreshCw className="animate-spin" size={32} /> : (
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-24 animate-slide-up">
      <div className="text-center mb-16">
        ...
        <h2 className="text-5xl font-black tracking-tighter mt-4 mb-4 text-slate-900 dark:text-white">Help & FAQ</h2>
        <p className="text-xl text-slate-500 font-bold">Everything you need to know about the SDG-INSIGHT framework.</p>
      </div>

      <div className="space-y-6">
        {faqs.map((faq, i) => (
          <div key={i} className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-xl border border-slate-100 dark:border-slate-800 transition-all hover:border-emerald-500">
            <h3 className="text-xl font-black text-slate-900 dark:text-white mb-3 flex items-start gap-3">
              <span className="text-emerald-500 mt-1"><HelpCircle size={20} /></span>
              {faq.q}
            </h3>
            <p className="text-slate-500 dark:text-slate-400 font-bold leading-relaxed ml-8">
              {faq.a}
            </p>
          </div>
        ))}
    ...
    <div className="max-w-2xl mx-auto px-8 py-24 animate-slide-up">
      <div className="bg-white dark:bg-slate-900 p-16 rounded-[5rem] shadow-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden">
        ...
        <header className="mb-14 text-center">
          ...
          <h2 className="text-5xl font-black tracking-tighter mt-6 text-slate-900 dark:text-white">Contact Us</h2>
          <p className="text-slate-500 font-bold mt-4">Need counseling or assistance? Send us an email and our team will help you out.</p>
        </header>
        ...
        {success ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="space-y-3">
              ...
              <input required type="text" className="w-full px-8 py-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all text-xl text-slate-900 dark:text-white" placeholder="John Doe" />
            </div>
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-6">Email Address</label>
              <input required type="email" className="w-full px-8 py-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all text-xl text-slate-900 dark:text-white" placeholder="john@example.com" />
            </div>
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-6">Message</label>
              <textarea required rows={4} className="w-full px-8 py-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all text-xl text-slate-900 dark:text-white resize-none" placeholder="How can we help you?"></textarea>
            </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-24 animate-slide-up">
      <div className="text-center mb-16">
        ...
        <h2 className="text-5xl font-black tracking-tighter mt-4 mb-4 text-slate-900 dark:text-white">About SDG-INSIGHT</h2>
        <p className="text-xl text-slate-500 font-bold">Empowering higher education institutions to drive sustainable global impact.</p>
      </div>

      <div className="bg-white dark:bg-slate-900 p-12 rounded-[3rem] shadow-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden mb-12">
        <div className="absolute top-0 right-0 w-1/2 h-full opacity-[0.03] pointer-events-none">
            <ContextImage prompt="global sustainability education network" className="w-full h-full" overlay={false} />
        </div>
        <div className="relative z-10 space-y-8 text-slate-600 dark:text-slate-300 font-bold leading-relaxed text-lg">
          <p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-50 dark:bg-slate-800/50 p-8 rounded-[2rem] text-center border border-slate-100 dark:border-slate-700">
          <Globe className="mx-auto mb-4 text-emerald-500" size={32} />
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 p-8 rounded-[2rem] text-center border border-slate-100 dark:border-slate-700">
          <BrainCircuit className="mx-auto mb-4 text-blue-500" size={32} />
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 animate-slide-up">
      <div className="flex justify-between items-center mb-12 no-print">
        ...
        <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter">Assessment Review</h2>
      </div>

      <div className="space-y-6">
        {reviewData.map((item, i) => (
          <div key={i} className={`p-10 rounded-[3rem] border-2 transition-all ${item.isCorrect ? 'bg-emerald-50/50 border-emerald-100 dark:bg-emerald-900/10 dark:border-emerald-900/30' : 'bg-rose-50/50 border-rose-100 dark:bg-rose-900/10 dark:border-rose-900/30'}`}>
            <div className="flex items-start gap-6">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-lg ${item.isCorrect ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
                {item.isCorrect ? <Check size={24} /> : <X size={24} />}
              </div>
              <div className="flex-1">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6 leading-tight">{item.question}</h3>
...
export default function App() {
  ...
  const handleSaveResult = async () => {
    if (!user || !lastResult || !aiInsights || isSaving) return;
    setIsSaving(true);
    try {
      const res = await backend.processAndSaveAssessment(
        user.id, 
        lastResult.dept, 
        lastResult.scores, 
        aiInsights
      ...
      // Automatic Background Saving of interaction
      if (user.role !== 'Admin') {
        backend.saveAiInteraction(user.id, `SDG Assessment: ${lastResult.dept}`, aiInsights);
      }
      
      alert("Assessment successfully archived to your institutional profile.");
    } catch (e) {
  ...
  return (
    <div className="min-h-screen flex flex-col selection:bg-emerald-200 selection:text-emerald-900 transition-colors duration-300">
      ...
      <main className="flex-grow">
        ...
        {view === 'result-summary' && <ResultSummary result={lastResult} insights={aiInsights} onContinue={() => setView('improvement')} onReview={() => setView('review')} onSave={handleSaveResult} isSaving={isSaving} autoSave={autoSave} onToggleAutoSave={setAutoSave} loading={loadingInsights} />}
    ...
    <div className="max-w-2xl mx-auto px-8 py-24 animate-slide-up">
      ...
      <div className="bg-white dark:bg-slate-900 p-16 rounded-[5rem] shadow-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-3">
            ...
            <input 
              ...
              className={`w-full px-8 py-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold outline-none border-2 border-transparent transition-all text-xl text-slate-900 dark:text-white ${user.role !== 'Guest' ? 'opacity-50 cursor-not-allowed' : 'focus:border-emerald-500'}`} 
            />
          ...
          <button disabled={loading || isDeleting} type="submit" className={`w-full py-6 rounded-3xl font-black text-xl shadow-2xl transition-all active:scale-95 ${success ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-900 dark:bg-emerald-600 text-white hover:bg-black'}`}>
        ...
        <div className="mt-16 pt-12 border-t border-slate-100 dark:border-slate-800 text-center">
          ...
          <p className="text-xs text-slate-400 font-bold mb-8">Permanently remove your identity and all associated data from the SDG-INSIGHT framework.</p>
          <button 
            onClick={handleDeleteAccount}
            disabled={isDeleting}
            className="px-10 py-5 bg-rose-50 dark:bg-rose-950/20 text-rose-600 dark:text-rose-400 rounded-3xl font-black text-xs uppercase tracking-widest hover:bg-rose-100 dark:hover:bg-rose-900/40 transition-all flex items-center justify-center gap-3 mx-auto active:scale-95"
          >
